...
class UsuariosController {
    ...
    public async listContacts(req: Request, res: Response): Promise<void> {
        const { id } = req.params;
        const resp = await pool.query("SELECT * FROM usuarios WHERE usuarios.id IN (SELECT contactos.id_contacto FROM contactos WHERE contactos.id_usuario = ?);", [id]);
        const resp2 = await pool.query("SELECT contactos.alias FROM contactos WHERE id_usuario = ?", [id]);
    ...
    public async listOneContact(req: Request, res: Response): Promise<void> {
        ...
        const resp = await pool.query("SELECT * FROM usuarios WHERE usuarios.id IN (SELECT contactos.id_contacto FROM contactos WHERE contactos.id_usuario = ?) AND usuarios.id = ?;", [id, id_contacto]);
        const resp2 = await pool.query("SELECT contactos.alias FROM contactos WHERE id_usuario = ? AND id_contacto = ?", [id, id_contacto]);
    ...
    public async newContact(req: Request, res: Response): Promise<void> {
        ...
        const existeAlias = await pool.query("SELECT * FROM contactos WHERE alias = ? AND id_usuario = ?", [req.body.alias, id]);
    ...
    public async updateContact(req: Request, res: Response): Promise<void> {
        ...
        if (existeAlias.length > 0) {
            ...
            const resp = await pool.query("UPDATE contactos set ? WHERE id_usuario = ? AND id_contacto = ?", [req.body, id, id_contacto]);
            res.json(resp);
        }
    }
    ...
    public async deleteContact(req: Request, res: Response): Promise<void> {
        const { id, id_contacto } = req.params;
        const resp = await pool.query("DELETE FROM contactos WHERE id_usuario = ? AND id_contacto = ?", [id, id_contacto]);
        res.json(resp);
    }
}
